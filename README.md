...
```
(Optional)
To train the model, begin by selecting appropriate hyperparameters. We group these hyperparameters into three components: model architecture, diffusion process, and training flags, largely following the training protocols introduced in [DDDM](https://github.com/adrita78/DDDM). The following defaults are chosen to provide a stable and effective baseline for training.
```bash
...
TRAIN_FLAGS="--schedule_sampler uniform --lr 3e-4 --min_lr 3e-5 \
--adam_beta1 0.9 --adam_beta2 0.95 --adam_eps 1e-6 \
--weight_decay 0.1 --lr_schedule cosine --warmup_steps 5000 \
