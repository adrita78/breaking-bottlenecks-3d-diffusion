...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        #if self.use_fp16:
            #self._setup_fp16()    # Only if you want fp16()
    ...
    def train_epoch(self, data, epoch):
        for i, batch in enumerate(data):
            self.run_step(batch, epoch)

            if i % self.log_interval == 0:
                logger.dumpkvs()

    def run_step(self, batch, epoch):
        self.forward_backward(batch, epoch)
    ...
    def forward_backward(self, batch, epoch):
        ...
        max_i = (total_graphs // self.microbatch) * self.microbatch

        for i in range(0, max_i, self.microbatch):
            ...
            last_batch = (i + self.microbatch) >= total_graphs

            t, weights = self.schedule_sampler.sample(
                micro_batch.ptr.size(0) - 1,
                dist_util.dev(),
            )
            ...
            self.model.update_xbar(output, m_index)

            loss_guide = (
                losses["guide"]
                .view(micro_batch.ptr.size(0) - 1, -1)
                * weights.unsqueeze(1)
            )
